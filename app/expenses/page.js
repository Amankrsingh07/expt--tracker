...
function Money({ amount }) {
  return <span className="font-bold text-lg">{formatCurrency(amount)}</span>;
}
...
function SummaryBar({ income = 0, expense = 0 }) {
  ...
  return (
    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden h-4 shadow-sm">
      <div className="h-4 bg-gradient-to-r from-green-400 to-green-500 rounded-full" style={{ width: `${Math.max(0, incomePct)}%` }} />
      <div className="h-4 bg-gradient-to-r from-red-400 to-red-500 rounded-full" style={{ width: `${Math.max(0, expensePct)}%`, marginLeft: `-${expensePct}%` }} />
    </div>
  );
}
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted mt-2">Track and manage all your expenses</p>
          </div>
          <Button asChild className="h-12 px-6 text-base font-bold bg-gradient-to-r from-blue-600 to-blue-500 hover:shadow-lg">
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 border-blue-200 dark:border-blue-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-2xl">📊 Summary</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="grid grid-cols-3 gap-4 flex-1">
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border-l-4 border-green-500">
                  <div className="text-sm text-muted font-semibold">Total Income</div>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border-l-4 border-red-500">
                  <div className="text-sm text-muted font-semibold">Total Expense</div>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border-l-4 border-blue-500">
                  <div className="text-sm text-muted font-semibold">Remaining</div>
                  <div className={`text-2xl font-bold mt-2 ${remaining >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-orange-600 dark:text-orange-400'}`}>
              ...
              <div className="flex-1 flex flex-col justify-center">
                <p className="text-sm font-semibold text-muted mb-3">Income vs Expense</p>
                <SummaryBar income={totalIncome} expense={totalExpense} />
                <p className="text-xs text-muted mt-2">
                  {totalIncome > 0 ? `${Math.round((totalExpense / totalIncome) * 100)}% of income spent` : 'No income data'}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-amber-200 dark:border-amber-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">🔍 Search & Filter</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Search */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <Label className="font-semibold">Search by description or category</Label>
                <Input
                  value={filters.search || ''}
                  onChange={(e) => setFilters({ ...filters, search: e.target.value })}
                  placeholder="🔎 Search expenses..."
                  className="h-10 mt-2 bg-white text-black 
             ...
             dark:border-slate-600"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 h-10 border-2 rounded-lg mt-2  bg-white text-black 
             dark:bg-slate-800 dark:text-white 
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.length > 0 ? (
              filtered.map((e) => (
                <Card key={e.id} className="hover:shadow-lg transition-all hover:scale-105 duration-300 border-l-4 border-red-400">
                  <CardContent className="p-5 space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-sm text-muted">
                          📅 {new Date(e.date).toLocaleDateString('en-IN', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                        </p>
                        <p className="text-lg font-bold mt-1">{e.category?.name || '❓ No Category'}</p>
                      </div>
              ...
              <div className="col-span-3 text-center py-12">
                ...
                <p className="text-lg text-muted font-semibold">No expenses found</p>
